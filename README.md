...
- **The reference chart's named regions** — "optimum pre-race", "not coping
  ...
  their own, and were never a formula even in the original source: an
  independent Python port of the same chart implements the five real zones but
  doesn't attempt these three at all. Their boundaries here come from
  pixel-measuring the actual reference image against its own printed axis
  ticks and the zone thresholds `classify()` already uses elsewhere — where
  that gives an exact answer (the angular span, and the split between
  "optimum" and "not coping" sitting exactly on the real Normal/LimitIntensity
